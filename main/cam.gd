...
func _input(event):
	if event is InputEventMouseMotion:
		...
		if event.ctrl_pressed:
			_zoom_at_screen_pos_delta(last_mouse_screen, event.delta.y)
		else:
			position += event.delta * pan_speed / zoom.x
	
	elif event is InputEventMagnifyGesture:
		_zoom_at_screen_pos_factor(last_mouse_screen, event.factor)


func _zoom_at_screen_pos_delta(screen_pos: Vector2, delta_y: float) -> void:
...
func _zoom_at_screen_pos_factor(screen_pos: Vector2, factor: float) -> void:
	var before := screen_to_world(screen_pos)

	zoom *= factor   # divide, not multiply
